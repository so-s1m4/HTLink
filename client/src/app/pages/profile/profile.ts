...
@Component({
  ...
  imports: [Block, CommonModule, Tag, ProjectPreview, RouterLink, ImgPipe, NgIcon],
...
export class Profile implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(async (params) => {
      ...
      if (
        _id === 'me' ||
        (!_id && this.profileService.me$()) ||
        _id === this.profileService.me$()?.id
      ) {
        this.isMy = true;
        this.data = {
          user: this.profileService.me$,
          projects: await this.projectsService
            .getProjectsByUserId(this.profileService.me$()?.id || 'me', 3)
            .then((res) => res.items),
        };
      } else {
        this.isMy = false;
        this.data = {
          user: await this.profileService.getProfileById(_id || '').then((res) => {
            if (!res) {
              this.data = null;
